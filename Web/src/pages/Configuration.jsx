...
export default function Configuration() {
  ...
  function fetchConfig() {
    fetch("http://localhost:4000/config").then((res) => {
      res.json().then((data) => {
        setTitle(data.title);
        ...
        // Marathon
        onTypeSpecificDataChanged(
          "marathon",
          "minimumTime",
          data.marathon.minimumTime
        );
        onTypeSpecificDataChanged(
          "marathon",
          "startTimesFile",
          data.marathon.startTimesFile
        );
        onTypeSpecificDataChanged(
          "marathon",
          "goalTimesFile",
          data.marathon.goalTimesFile
        );
        ...
        onTypeSpecificDataChanged(
          "lap",
          "timeForMassStart",
          data.lap.timeForMassStart
        );
        onTypeSpecificDataChanged(
          "lap",
          "startTimesFile",
          data.lap.startTimesFile
        );
        onTypeSpecificDataChanged(
          "lap",
          "goalTimesFiles",
          data.lap.goalTimesFiles[0]
        );
  ...
  return (
    <form onSubmit={onSubmit}>
      <Center>
        ...
        >
          {/* Competition title */}
          <Box height="30px">Tävlingstitel</Box>
          <Box height="30px">
            <Input
              size="md"
          ...
          <Box height="30px">Footer-text för resultatfil</Box>
          <Box height="30px">
            <Input
              size="md"
              placeholder="Textexempel i footer"
              value={footer}
          ...
          <Box height="30px">Filväg till resultatfil i server</Box>
          <Box height="30px">
            <Input
              size="md"
              value={resultFile}
          ...
          <Box height="30px">Filväg till namnfil i server</Box>
          <Box height="30px">
            <Input
              size="md"
              value={nameFile}
          ...
          {/* Sort result */}
          <Box height="30px">Sortering av resultat</Box>
          <Box height="30px">
            <Switch
              size="lg"
              colorScheme="yellow"
              value={sorting}
              onChange={(event) => setSorting(!sorting)}
            />
          </Box>

          {/* Choose competition type */}
          <Box height="30px">Tävlingstyp</Box>
          <Box height="30px">
          ...
          {type === "lap" ? (
            ...
            <React.Fragment>
              ...
              <Box height="30px">Minimumtid per varv</Box>
              <Box height="30px">
                <Input
                  size="md"
                  placeholder="Använd formatet hh:mm:ss"
                  value={typeSpecificData[type].minimumTime}
                  onChange={(event) =>
                    onTypeSpecificDataChanged(
                      type,
                      "minimumTime",
              ...
              <Box height="30px">Mass-start</Box>
              <Box height="30px">
                <Switch
                  size="lg"
                  colorScheme="yellow"
                  value={typeSpecificData[type].massStart}
                  onChange={(event) =>
                    setSorting(!typeSpecificData[type].massStart)
                  }
                />
              </Box>

              {/* Time for mass start */}
              <Box height="30px">Tid för mass-start</Box>
              <Box height="30px">
                <Input
                  size="md"
                  placeholder="Använd formatet hh:mm:ss"
                  value={typeSpecificData[type].timeForMassStart}
                  onChange={(event) =>
                    onTypeSpecificDataChanged(
                      type,
                      "timeForMassStart",
              ...
              {/* Start time file */}
              <Box height="30px">Filväg till fil med starttider i server</Box>
              <Box height="30px">
                <Input
                  size="md"
                  value={typeSpecificData[type].startTimesFile}
                  onChange={(event) =>
                    onTypeSpecificDataChanged(
                      type,
                      "startTimesFile",
              ...
              {/* End time file */}
              <Box height="30px">Filväg till fil med måltider i server</Box>
              <Box height="30px">
                <Input
                  size="md"
                  placeholder="T.ex: fil1.txt"
                  value={typeSpecificData[type].goalTimesFiles}
                  onChange={(event) =>
                    onTypeSpecificDataChanged(
                      type,
                      "goalTimesFiles",
              ...
              {/* Race end time */}
              <Box height="30px">Sluttid för lopp</Box>
              <Box height="30px">
                <Input
                  size="md"
                  placeholder="Använd formatet hh:mm:ss"
                  value={typeSpecificData[type].raceEndTime}
                  onChange={(event) =>
                    onTypeSpecificDataChanged(
                      type,
                      "raceEndTime",
            ...
            <React.Fragment>
              ...
              <Box height="30px">Minimumtid för helt lopp</Box>
              <Box height="30px">
                <Input
                  size="md"
                  placeholder="Använd formatet hh:mm:ss"
              ...
              <Box height="30px">
                <Input
                  size="md"
                  value={typeSpecificData[type].goalTimesFile}
                  onChange={(event) =>
                    onTypeSpecificDataChanged(
                      type,
                      "goalTimesFile",
